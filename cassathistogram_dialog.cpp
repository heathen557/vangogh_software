...
*@brief:  cassat直方图
*/
void cassatHistogram_Dialog::toShowCassatt_histogramSlot(QVector<double> numData,int yMax)   //显示卡萨特直方图
{

    QSharedPointer<QCPAxisTickerText> textTicker(new QCPAxisTickerText);
    textTicker->setTicks(cassatt_HistorgramTicks, cassatt_HistorgramLabels);
    ui->cassatHistogram_widget->xAxis->setTicker(textTicker);
    ui->cassatHistogram_widget->xAxis->setRange(0,binSize);
    ui->cassatHistogram_widget->yAxis->setRange(0,yMax);
    regen_cassatt->setData(cassatt_HistorgramTicks, numData);        //只不过第一个向量xTicks的每个元素表示“第几个柱子”，然后后面对应的values表示对应“柱子的值”
    ui->cassatHistogram_widget->replot();
}
/**
*@date 2020-04-30
*@brief: 4300直方图
*/
void cassatHistogram_Dialog::toShow4300_histogramSlot(QVector<double> numData,int yMax)      //显示4300直方图
{
    QSharedPointer<QCPAxisTickerText> textTicker(new QCPAxisTickerText);
    textTicker->setTicks(vi4300_HistorgramTicks, vi4300_HistorgramLabels);
    ui->VI4300Histogram_widget->xAxis->setTicker(textTicker);
    ui->VI4300Histogram_widget->xAxis->setRange(0,binSize);
    ui->VI4300Histogram_widget->yAxis->setRange(0,yMax);
    regen_4300->setData(vi4300_HistorgramTicks, numData);        //只不过第一个向量xTicks的每个元素表示“第几个柱子”，然后后面对应的values表示对应“柱子的值”
    ui->VI4300Histogram_widget->replot();
}


void cassatHistogram_Dialog::on_pushButton_clicked()
{
    QVector<double> vec(binSize);
    for(int i=0;i<binSize;i++)
    {
        vec[i] = i;
    }
    ...
    toShowVangogh_histogramSlot(0,vec,yMax);
    toShowCassatt_histogramSlot(vec,yMax);
    toShow4300_histogramSlot(vec,yMax);
